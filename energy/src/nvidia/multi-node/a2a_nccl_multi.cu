...
void run(nvidia::utils::ncclContext& ctx){ 
    ...
    cudaMalloc((void **)&d_sendbuf, buff_size_byte[num_iters - 1] /  size);
    cudaMalloc((void **)&d_recvbuf, buff_size_byte[num_iters - 1]); // each rank at the end will have 1GB of data

    T *h_sendbuf = (T *)malloc(buff_size_byte[num_iters - 1] / size);
    T *h_recvbuf = (T *)malloc(buff_size_byte[num_iters - 1]);

    cudaStreamCreate(&stream);

    for (int i = 0; i < WARM_UP_RUN; i++) {
        size_t count_el = (buff_size_byte[num_iters-1] / sizeof(T) ) /  size;
        
        cudaMemcpy(h_sendbuf, d_sendbuf, buff_size_byte[num_iters-1]  /  size, cudaMemcpyDeviceToHost);
    ...
    cudaMemset(d_sendbuf, rank, buff_size_byte[num_iters-1] / size); // initialize the send buffer with the rank value for each byte. Each rank will have a different value in the send buffer
    auto mem_cpy_t_start = std::chrono::high_resolution_clock::now();
    cudaMemcpy(h_sendbuf, d_sendbuf, buff_size_byte[num_iters-1] / size, cudaMemcpyDeviceToHost);
    auto mem_cpy_t_end = std::chrono::high_resolution_clock::now();

    for (int i = 0; i < num_iters; i++) {
        size_t count_el = (buff_size_byte[i] / sizeof(T)) / size;
        int chain_size = 0;
