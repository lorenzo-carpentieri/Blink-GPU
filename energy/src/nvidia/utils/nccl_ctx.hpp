...
namespace nvidia {
    namespace utils {
        ...
        struct ncclContext {
            ...
            std::unique_ptr<cudaStream_t> stream; // cuda stream associated to the rank
        ...
        int pick_device_for_rank(nvidia::utils::ncclContext& ctx) {
            int numGPUs;
            ...
            cudaSetDevice(local_rank % numGPUs); // Set device
            return local_rank % numGPUs;
        }
        ...
        inline nvidia::utils::ncclContext init_nccl(const std::string& output_dir,
            ...
            // Init ncclContext with rank info                                    
            nvidia::utils::ncclContext ctx;
			...
			ctx.stream = std::make_unique<cudaStream_t>(stream);
