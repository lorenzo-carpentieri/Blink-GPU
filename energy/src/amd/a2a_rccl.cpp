...
void run(amd::utils::rcclContext& ctx){ 
    ...
    std::cerr<< "###### Start wramup ... ######" <<std::endl;
    ...
    for (int i = 0; i < num_iters; i++) {
        std::cerr << "Iter " << i << ", message size " << buff_size_byte[i] <<std::endl;
        ...
        for (int run = 0; run < MAX_RUN; run++) {
            ...
            while (a2a_time < (TIME_TO_ACHIEVE_MS * 1000)) { // a2a_time in microseconds
                ...
                size_t a2a_time_max = std::chrono::duration_cast<std::chrono::microseconds>(end_s - start_s).count();

                MPI_Allreduce(MPI_IN_PLACE, &a2a_time_max, 1, MPI_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);
                a2a_time+=a2a_time_max;
                chain_size++;
            }
            powerProf.stop();
...
int main(int argc, char *argv[]) {
    ...
    std::cerr<< "####### Start init context ... ########" <<std::endl;
    amd::utils::rcclContext ctx = amd::utils::init_rccl(csv_path, "a2a"); // initialize oneCCL and MPI
    std::cerr<< "####### End init context ... #######" <<std::endl;
