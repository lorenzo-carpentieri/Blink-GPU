...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < iterations; i++) {
        if (rank == 0) {
            CHECK_RCCL(ncclSend(sendbuff, count, ncclInt8, 1, comm, stream));
            CHECK_RCCL(ncclRecv(recvbuff, count, ncclInt8, 1, comm, stream));
        } else {
            CHECK_RCCL(ncclRecv(recvbuff, count, ncclInt8, 0, comm, stream));
            CHECK_RCCL(ncclSend(sendbuff, count, ncclInt8, 0, comm, stream));
        }
        // IMPORTANT: enforce completion before next iteration
        CHECK_HIP(hipStreamSynchronize(stream));
    }
    ...
    if (rank == 0) {
        ...
        printf("Unidirectional goodput: %.2f GB/s\n", gbps);
    }
