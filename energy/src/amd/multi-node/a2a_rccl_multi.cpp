...
void run(amd::utils::rcclContext& ctx){ 
    ...
    hipStream_t stream = std::move(*ctx.stream);
    ...
    CHECK_HIP(hipMalloc((void **)&d_recvbuf, buff_size_byte[num_iters - 1] * size));
...
int main(int argc, char *argv[]) {
    ...
    amd::utils::rcclContext ctx = amd::utils::init_rccl(csv_path, "a2a");
