...
sycl_env_vars = {"CCL_SYCL_COPY_ENGINE": "0", 
            ...
            "CCL_SYCL_AUTO_USE_TMP_BUF" : "1",
            "CCL_TOPO_ALGO" : "1"}

lz_env_vars = {"CCL_SYCL_COPY_ENGINE" : "1", 
...
a2a_sycl_env_vars = {"CCL_ALLTOALL": "topo", "CCL_ALLTOALLV_MONOLITHIC_KERNEL": "1", "CCL_ALLTOALLV_MONOLITHIC_READ_KERNEL": "1"}
a2a_lz_env_vars = {"CCL_ALLTOALL": "topo", "CCL_ALLTOALLV_MONOLITHIC_KERNEL": "0", "CCL_ALLTOALLV_MONOLITHIC_READ_KERNEL": "0"}
...
def set_env(pbs_file_to_modify, coll, coll_path):
    path_name = coll_path.lower()
    if path_name == "sycl":
        selected_env_vars = dict(sycl_env_vars)
        if coll == "a2a":
            selected_env_vars.update(a2a_sycl_env_vars)
    elif path_name == "lz":
        selected_env_vars = dict(lz_env_vars)
        if coll == "a2a":
            selected_env_vars.update(a2a_lz_env_vars)
    else:
...
def set_pbs(pbs_file_to_modify, log_dir, num_nodes, coll, partition, pbs_time, coll_path):
...
def main():
    ...
    parser.add_argument("--path", required=True, default="00:10:00", help="Specify the oneCCL collective path: sycl or lz")
    ...
    set_pbs(str(generated_pbs_path), args.log, num_nodes, args.coll, args.partition, args.time, args.path)
    set_env(str(generated_pbs_path), args.coll, args.path)
...
def submit_job(pbs, exe, csv_log, coll, scaleup_mode, alg_name, nic_mode,env):
    env_vars = {
        "EXE_PATH": exe,
        "CSV_LOG_DIR": csv_log,
        "COLLECTIVE": coll,
        "SCALEUP_MODE": scaleup_mode,
        "SCALEOUT_MODE": alg_name,
        "NIC_MODE": nic_mode,
        "SET_ENV_PATH": env,
    }
